...
const TeacherPlanning = () => {
    ...
    const [feedbackModal, setFeedbackModal] = useState({ open: false, student: null });
    const [feedbackText, setFeedbackText] = useState("");
    const [isSavingTeam, setIsSavingTeam] = useState(false);

    const handleSaveTeamFeedback = (teamId, text) => {
        setTeams(prev => prev.map(t => t.id === teamId ? { ...t, teamFeedback: text } : t));
    };

    const handleManualSaveTeamFeedback = () => {
        setIsSavingTeam(true);
        handleSaveTeamFeedback(selectedTeam.id, selectedTeam.teamFeedback);
        setTimeout(() => setIsSavingTeam(false), 1500);
    };

    const handleOpenStudentFeedback = (student) => {
        setFeedbackText(student.feedback || "");
        setFeedbackModal({ open: true, student });
    };

    const handleSaveStudentFeedback = () => {
        if (selectedTeam && feedbackModal.student) {
            const updatedMembers = selectedTeam.members.map((m) =>
                m.id === feedbackModal.student.id ? { ...m, feedback: feedbackText } : m
            );
            ...
            setFeedbackModal({ open: false, student: null });
        }
    };
    ...
    if (selectedTeam) {
        return (
            <div className="max-w-5xl mx-auto">
                ...
                >
                    <ChevronLeft size={20} /> Voltar para Equipes
                </button>

                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h2 className="text-3xl font-bold text-slate-800">{selectedTeam.name}</h2>
                        <p className="text-slate-500">Projeto: {selectedTeam.project}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 h-fit">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="font-bold text-lg text-slate-800">Feedback da Equipe</h3>
                        </div>
                        <p className="text-sm text-slate-500 mb-2">Avalie o desempenho coletivo, colaboração e entregas do grupo.</p>
                        <textarea
                            className="w-full h-40 p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-slate-700 resize-none text-sm"
                            placeholder="Escreva o feedback geral para a equipe aqui..."
                            value={selectedTeam.teamFeedback}
                            onChange={(e) => {
                                const val = e.target.value;
                                setSelectedTeam({ ...selectedTeam, teamFeedback: val });
                            }}
                        />
                        <div className="mt-3 flex justify-between items-center">
                            <span className="text-xs text-slate-400 font-medium">
                                {selectedTeam.teamFeedback.length} caracteres
                            </span>
                            <button
                                onClick={handleManualSaveTeamFeedback}
                                className={`px-4 py-2 rounded-lg text-sm font-bold transition flex items-center gap-2 ${isSavingTeam ? 'bg-green-100 text-green-700' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}
                            >
                                {isSavingTeam ? <Check size={16} /> : <Save size={16} />}
                                {isSavingTeam ? "Salvo!" : "Salvar Feedback"}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {selectedTeam.members.map((member) => (
                            <div key={member.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition flex items-center justify-between">
                                <div className="flex items-center gap-3">
                ...
                {feedbackModal.open && feedbackModal.student && (
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100]">
                        <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
                            <div className="flex justify-between items-center mb-6">
                                <div className="flex items-center gap-3">
                                    <img src={feedbackModal.student.avatar} className="w-10 h-10 rounded-full" />
                                    <div>
                                        <h3 className="font-bold text-slate-800">Feedback Individual</h3>
                                        <p className="text-xs text-slate-500">{feedbackModal.student.name}</p>
                                    </div>
                                </div>
                                <button onClick={() => setFeedbackModal({ open: false, student: null })}>
                                    <X size={20} className="text-slate-400 hover:text-slate-600" />
                                </button>
                            </div>

                            <textarea
                                className="w-full h-40 p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-slate-700 resize-none text-sm mb-4"
                                placeholder={`Escreva o feedback específico para ${feedbackModal.student.name}...`}
                                value={feedbackText}
                            ...
                            <div className="flex justify-end gap-2">
                                <button
                                    onClick={() => setFeedbackModal({ open: false, student: null })}
                                ...
                                <button
                                    ...
                                    className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-bold transition shadow-lg shadow-indigo-200"
                                >
                                    Salvar Feedback
                                </button>
                            </div>
    ...
    return (
        <div>
            <h2 className="text-3xl font-bold text-slate-800 mb-2">Planejamento & Feedback</h2>
            <p className="text-slate-500 mb-8">Acompanhe e avalie o desempenho das equipes de projeto.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {teams.map(team => (
                    <div
                        ...
                        className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition cursor-pointer group relative overflow-hidden"
                    >
                        <div className="absolute top-0 left-0 w-2 h-full bg-indigo-500 group-hover:bg-indigo-600 transition-colors"></div>
                        <div className="pl-4">
                            <h3 className="text-xl font-bold text-slate-800 mb-1 group-hover:text-indigo-600 transition-colors">{team.name}</h3>
                            <p className="text-sm text-slate-500 mb-4 font-medium bg-slate-100 inline-block px-2 py-0.5 rounded text-xs">{team.project}</p>

                            <div className="flex items-center justify-between mt-4 border-t border-slate-50 pt-4">
                                <div className="flex -space-x-2">
                                    {team.members.map(m => (
                                        <img key={m.id} src={m.avatar} className="w-8 h-8 rounded-full border-2 border-white bg-slate-200" title={m.name} />
                                    ))}
                                </div>
                            ...
                            <div className="mt-4 flex gap-2">
                                <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded border ${team.teamFeedback ? 'bg-green-50 text-green-700 border-green-200' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                                    {team.teamFeedback ? "Feedback Equipe OK" : "Feedback Pendente"}
                                </span>
                            </div>
                        </div>
                    </div>
...
const TeacherPerformance = () => {
    const [students] = useState(MOCK_STUDENT_PERFORMANCE);
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <div className="flex gap-3">
                                    <span className={`px-3 py-1 rounded-full text-sm font-bold bg-white/20 backdrop-blur-sm`}>
                                        {selectedStudent.projects} Projetos
                                    </span>
                                    <span className={`px-3 py-1 rounded-full text-sm font-bold ${statusInfo.color} border`}>
                        ...
                        <div className="text-right">
                            <div className="bg-white/20 backdrop-blur-sm px-6 py-4 rounded-2xl">
                                <p className="text-xs font-bold opacity-80 uppercase mb-1">Média Geral</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                        ...
                        <p className="text-xs text-slate-500">
                            {trend === 'up' ? '↗' : '↘'} {Math.abs(selectedStudent.grades[selectedStudent.grades.length - 1] - selectedStudent.grades[0]).toFixed(1)} pontos
                        </p>
                    </div>
                </div>
                ...
                {selectedStudent.status === 'at-risk' && (
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <button className="p-4 border-2 border-indigo-200 rounded-xl hover:bg-indigo-50 transition text-left">
                                <p className="font-bold text-indigo-600 mb-1">📞 Contatar Responsáveis</p>
                                <p className="text-sm text-slate-600">Agendar reunião para discutir desempenho</p>
                            </button>
                            <button className="p-4 border-2 border-purple-200 rounded-xl hover:bg-purple-50 transition text-left">
                                <p className="font-bold text-purple-600 mb-1">📚 Reforço Extra</p>
                                <p className="text-sm text-slate-600">Indicar aulas de apoio ou monitoria</p>
                            </button>
                            <button className="p-4 border-2 border-blue-200 rounded-xl hover:bg-blue-50 transition text-left">
                                <p className="font-bold text-blue-600 mb-1">👥 Mentoria de Pares</p>
                                <p className="text-sm text-slate-600">Conectar com aluno de bom desempenho</p>
                            </button>
                            <button className="p-4 border-2 border-green-200 rounded-xl hover:bg-green-50 transition text-left">
                                <p className="font-bold text-green-600 mb-1">✅ Plano de Ação</p>
                                <p className="text-sm text-slate-600">Criar metas específicas e acompanhamento</p>
                            </button>
                        </div>
                    </div>
                )}
        ...
        <div>
            <div className="flex justify-between items-end mb-8">
                <div>
                    ...
                    <p className="text-slate-500">Análise detalhada e comparação entre turmas.</p>
                </div>
                <select
                    value={selectedClass}
                    onChange={(e) => setSelectedClass(e.target.value)}
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-2xl font-bold text-slate-800">
                                {students.filter(s => s.status === 'improving').length}
                            </p>
                        </div>
                    </div>
                </div>
