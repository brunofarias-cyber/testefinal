/*
═══════════════════════════════════════════════════════════════════════
  SCRIPT DE SEED - BANCO DE DADOS BNCC
═══════════════════════════════════════════════════════════════════════

Este arquivo popula o banco de dados com:
1. Tabelas de BNCC (competências e habilidades)
2. Relacionamento entre Projetos e BNCC

Executar em PostgreSQL/Neon:
  psql -U seu_usuario -d seu_banco -f 01_SEED_BNCC_DATABASE.sql

Ou copiar cada seção individualmente no seu client SQL.
...
-- Teste 1: Listar todas as áreas
-- SELECT * FROM bncc_areas;

-- Teste 2: Listar habilidades de Matemática
-- SELECT * FROM v_bncc_habilidades_completo WHERE area_codigo = 'MAT';

-- Teste 3: Buscar habilidades por nível escolar
-- SELECT DISTINCT ano_escolar FROM bncc_habilidades;

-- Teste 4: Contar habilidades por área
-- SELECT a.nome, COUNT(*) as qtd FROM bncc_habilidades h
-- JOIN bncc_areas a ON h.area_id = a.id
-- GROUP BY a.nome;
...
-- ☐ Tabela bncc_areas criada com 5 áreas
-- ☐ Tabela bncc_competencias_gerais criada com 10 competências
-- ☐ Tabela bncc_habilidades criada com ~15 habilidades
-- ☐ Tabela project_bncc_habilidades criada (vazia por enquanto)
-- ☐ Tabela project_wizard_draft criada (vazia por enquanto)
-- ☐ Índices criados
-- ☐ Views criadas

-- PRÓXIMOS PASSOS:
-- 1. Ampliar bncc_habilidades com 100+ habilidades reais
-- 2. Criar endpoints backend para buscar dados
-- 3. Criar componente React do Wizard
-- 4. Integrar com API de IA (OpenAI/Claude)
